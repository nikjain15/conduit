...
| C3 | P1 | No API versioning or breaking change policy. The HTTP surface is `/v1/*` (`services/gateway/src/router.ts:51-60`) and every package sits at 0.1.0, but `PUBLISHING.md` says only "Versions stay at 0.1.0. Bump per package before republishing", with no definition of what counts as breaking or what `/v1` promises | `docs/VERSIONING.md`: what `/v1` guarantees, semver rules per package, and the deprecation window | **Closed 2026-08-02.** `docs/VERSIONING.md` defines what counts as breaking (and what does not), what `/v1` promises, a six-month deprecation window with `Deprecation`/`Sunset` headers and a dated notice schedule, and per-package semver from the first publish. Enforced by `scripts/check-api-surface.mjs` against a committed snapshot in CI, so a removed or renamed field fails the build rather than a consumer. `CHANGELOG.md` added. |
